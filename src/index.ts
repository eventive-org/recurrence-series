...
export default class Series {
  ...
  setRecurrence(rruleString: string) {
    return this.setRecurrenceAndDurations([{ rrule: rruleString }])
  }
  setRecurrenceAndDurations(list: { rrule?: string; duration?: number }[]) {
    this._updated = []
    this._created = []
    this._deleted = []
    let kept = []
    ...
    for (const { rrule, duration } of list) {
      if (rrule) {
        ...
        let repeats = rrulestr(rrule).all((date, i) => i < this._options.maxRepeats)
        ...
        if (repeats.length > 0) {
          for (let repeat of repeats) {
            ...
            let end = moment(repeat)
              .add(this._options.length)
              .toDate()
  ...
  setLength(length: number) {
    ...
    return this.setRecurrenceAndDurations([{ duration: length }])
  }
  split(date: Date) {
